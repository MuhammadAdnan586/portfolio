...
import { View, Text, FlatList, TextInput, TouchableOpacity, StyleSheet, Animated, Alert } from 'react-native';
import { useContext } from 'react';
import { SettingsContext } from '../context/SettingsContext';
import { translate } from '../utils/lang.js'; // Aapka naya lang file

// Simple component to simulate the voice recording visual feedback 
...
export default function ChatbotScreen() {
    ...
    // Initial messages ko translate karein
    const initialBotMessage = translate('Initial Bot Message', language);
    ...
    // Timer logic for recording visual feedback
    useEffect(() => {
        if (isRecording) {
    ...
    const sendMessage = () => {
        ...
        // Simulated AI Response (using translation)
        setTimeout(() => {
            setMessages(prev => [...prev, { 
                id: Date.now().toString(), 
                text: translate('Simulated AI Scan Suggestion', language), 
                sender: 'bot' 
            }]);
        }, 800);
    };
    ...
    const stopAndSendVoice = () => {
        ...
        setMessages(prev => [
            ...
            { 
                ...
                text: `${translate('Voice Note', language)} (${duration}s) - ${translate('Sent for analysis.', language)}`, 
                sender: 'user' 
            }
        ]);
        ...
        setTimeout(() => {
            setMessages(prev => [...prev, { 
                ...
                text: translate('Simulated AI Analysis Response', language), 
                sender: 'bot' 
            }]);
        }, 800);
    };
    ...
    // Custom render item to apply RTL/LTR styles to message text
    const renderMessageItem = ({ item }) => {
        return (
            <View style={[styles.bubble, item.sender === 'bot' ? styles.bot : styles.user]}>
                <Text style={[{ color: item.sender === 'bot' ? '#fff' : '#000' }, textAlignmentStyle]}>{item.text}</Text>
            </View>
        );
    };
                ...
                {isRecording ? (
                    ...
                    <View style={styles.textBox}>
                        <TextInput
                            ...
                            placeholder={translate('Type your question...', language)} // Translate placeholder
...
const styles = StyleSheet.create({
    ...
    bot: { backgroundColor: '#2e7d32', alignSelf: 'flex-start' }, 
...
// recordingStyles remain unchanged.
